...
public class ZLauncher : Gtk.Window  {
	...
	public void initialize_zconfigs(){
		...
		try {
			...
			if (!icon_directory.query_exists ()){
				...
				File default_icon_directory = File.new_for_path(zconfig_icon_directory_fallback);	
	...
	public void load_logo(string path){
		assert (path.length>0);
		try {
			set_default_icon_from_file(path);
		} catch (Error e) {
			stderr.printf ("Error loading logo: %s\n", e.message);
		}
	}
